...
namespace App\GUI\Views\Dashboard;
...
class DashboardView extends AbstractView
{
    ...
    private function renderUserMetaInfo(User $user): string
    {
        ...
        if ($user->isStudent()) {
            ...
            if ($user->getMajor()) {
                $html .= '<span>Parcours : ' . $user->getMajor() . '</span>';
            }
        } elseif ($user->isProfessor()) {
            $html .= '<span>Département : Informatique</span>';
        } elseif ($user->isClient()) {
            $html .= '<span>Entreprise : ' . $user->getOrganisation() . ' </span>';
        }
    ...
    private function renderSAECard(User $user, SaeSubject $sae): string
    {
        ...
        $html .= '<div class="sae-icon" aria-hidden="true">' . (string) $sae->getId() . '</div>';
        $html .= '</div>';
        $html .= '<div class="sae-body">';
        $html .= '<h3>' . $sae->getName() . '</h3>';

        if (!empty($sae->getResponsibleProfessorId())) {
            $html .= '<p><strong>Enseignant :</strong> ' . $this->data['sae'][$sae->getId()] . '</p>';
        }

        $html .= '<div class="sae-actions">';
        $html .= '<a href="/sae/' . intval($sae->getId()) . '" class="btn btn-primary">Voir détails</a>';
