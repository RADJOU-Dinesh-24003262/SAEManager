...
namespace App\GUI\Views\ToDoList;
...
class ToDoListView extends BaseSaeView
{
    ...
    private function renderGroupList(array $groups): string
    {
        ...
        if (empty($groups)) {
            ...
            foreach ($groups as $groupData) {
                ...
                $saeId = $this->data['sae']['subject']->getId();
     ...
     * @param array<int, ToDoList> $tasks   The list of tasks.
     * @return string HTML content.
     */
     ...
     *
     * @param ToDoList $task Task entity.
     * @return string HTML for the task item.
     */
    private function renderTaskItem(ToDoList $task): string
    {
        ...
        $html .= '<span class="task-text">' . htmlspecialchars($task->getDescription()) . '</span>';
        $html .= '</label>';
        $html .= '</div>';
