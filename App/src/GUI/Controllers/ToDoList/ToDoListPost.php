...
use Override;
use App\Application\Validation\ToDoList\ToDoListValidator;
...
class ToDoListPost extends BaseController
{
    ...
    public function control(): void
    {
        ...
        try {
            ...
            if (!$canAccessUseCase->execute($this->user, $saeId)) {
     ...
     * @return integer
     * @throws AccessDeniedException If access to the SAE is denied.
     */
    private function determineTargetGroupId(User $user, int $saeId, array $input): int
    {
        $useCase = new GetSaeDetailsUseCase(new PdoSaeRepository(), new PdoSaeGroupRepository());
        $saeData = $useCase->execute($saeId, $user);

        if (!$saeData) {
            throw new AccessDeniedException("Accès non autorisé à cette SAE.", 403);
        }

        if ($user->isStudent()) {
            if (empty($saeData['groups'])) {
                throw new AccessDeniedException("Vous n'êtes assigné à aucun groupe.", 403);
            }
            $groupData = reset($saeData['groups']);
            return (int) $groupData['group']->getId();
        }

        // Professors can view but NOT add/modify via this controller.
        throw new AccessDeniedException("Seuls les étudiants peuvent gérer les tâches.", 403);
    }
     ...
     * @throws AccessDeniedException If the user access to the group is denied.
     * @return void
     */
    private function checkGroupAccess(User $user, int $saeId, int $groupId): void
    {
        ...
        if (isset($saeData['groups'])) {
            foreach ($saeData['groups'] as $groupData) {
                if ($groupData['group']->getId() == $groupId) {
                    $hasAccess = true;
                    break;
                }
            }
        ...
        if (!$hasAccess) {
            throw new AccessDeniedException("Vous n'avez pas accès à ce groupe.", 403);
        }
    }
    ...
    private function handleAdd(int $groupId, array $input): void
    {
        ...
        $task = new ToDoList($groupId, $description, $priority);
        $newId = $repo->save($task);
