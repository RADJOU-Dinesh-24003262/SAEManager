<?php

namespace Controllers\LegalTermsConditions;

use Views\LegalTermsConditions\LegalTermsConditionsView;
use Core\Controllers\ControllerInterface;
use Override;

/**
 * This class controls the legal terms and conditions page.
 *
 * @category   Controllers
 * @package    Src
 * @subpackage Controllers/LegalTermsConditions
 * @author     Alexandre Benhafessa <alexandre.benhafessa@etu.univ-amu.fr>
 * @author     François Dargentolle <francois.dargentolle@etu.univ-amu.fr>
 * @author     William Edelstein <william.edelstein@etu.univ-amu.fr>
 * @author     Nathan Griguer <nathan.griguer@etu.univ-amu.fr>
 * @author     Dinesh Radjou <dinesh.radjou@etu.univ-amu.fr>
 * @license    MIT License https://opensource.org/licenses/MIT
 * @link       https://github.com/RADJOU-Dinesh-24003262/SAEManager
 */
class LegalTermsConditionsController implements ControllerInterface
{
    /**
     * Principal manager of the controller
     *
     * @return void
     */
    #[Override]
    public function control(): void
    {
        $view = new LegalTermsConditionsView();
        $view->render();
    }

    /**
     * Check if this controller can handle the request
     *
     * @param  string $path   The requested URI path.
     * @param  string $method The HTTP method used in the request.
     * @return boolean True if the path is "/legal-terms-conditions" and the method is GET.
     */
    #[Override]
    public static function support(string $path, string $method): bool
    {
        return $path === "/legal-terms-conditions" && $method === "GET";
    }
}
